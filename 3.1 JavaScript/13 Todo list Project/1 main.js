...
function attachEventListeners(li,task){
  ...
  const enitBtn = li.querySelector(".edit-btn");
 ...
 enitBtn.addEventListener("click",function(){
  handleEdit(task.id,li);
 })

 checkbox.addEventListener("change",function(){
  console.log("complete",checkbox.checked);

  toggleTaskComplete(task.id, li, checkbox.checked);
...
function toggleTaskComplete(taskId, li, icCompleted){
  const tasks = getTasksFormLocalStorage();

  const task = tasks.find(task => task.id == taskId);

  if(task){
    task.completed =  icCompleted;
    localStorage.setItem("tasks",JSON.stringify(tasks));
    li.classList.toggle("completed",icCompleted)
  }
