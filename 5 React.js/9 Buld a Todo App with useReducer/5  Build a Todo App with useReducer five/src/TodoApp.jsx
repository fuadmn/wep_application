    ...
    <ul>
      {
        state.map( todo => (
          <li key={todo.id}>
             <span style={{textDecoration: todo.completed ? 'line-through' : "none"}} onClick={() => dispatcch({type:"toggle",payload: todo.id})}>{todo.text}</span>
             <button onClick={()=> dispatcch({type:"delete",payload: todo.id})}>delete</button>
          </li>
        ))
      }
    </ul>
